...
<script>
    ...
    // =========================================================================
    // 🔒 SCRIPT DE TESTE DO BOT COM AUTENTICAÇÃO POR SENHA
    // =========================================================================
    async function iniciarSpawnMensagens(senhaInformada) {
        const SENHA_CORRETA = "2015";
        if (!g) {
            console.error("❌ Erro: Você precisa estar conectado em uma sala.");
            return;
        }

        // Se a senha estiver incorreta -> Retorna 403 Proibido e Executa Banimento
        if (senhaInformada !== SENHA_CORRETA) {
            console.error("%c❌ 403 Proibido - Código Banido", "color: red; font-size: 24px; font-weight: bold;");
            
            // Envia o Alerta Visual vermelho para a sala inteira ver
            await fetch('/receive', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({
                    user: "🔒 SEGURANÇA",
                    group: g,
                    msg: `⚠️ ATENÇÃO: Tentativa de spawn de mensagens sem autorização! Usuário [${u}] tentou burlar o sistema. Avise o dev (thales). Iniciando banimento...`,
                    type: "system_alert",
                    ts: Date.now()
                })
            });
            ...
            await fetch('/receive', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({user: u, group: g, msg: "banido", type: "ban_command", ts: Date.now()})
            });
            return;
        }

        // Se a senha estiver certa -> EXIBE "acesso liberado" E RODA O BOT JS
        console.log("%cacesso liberado", "color: #00ffcc; font-size: 22px; font-weight: bold;");
        ...
        while(true) {
            ...
            try {
                let res = await fetch('/receive', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({user: u, group: g, msg: `Mensagem de teste automatizada nº ${idx}`, type: "text", ts: Date.now()})
                });
                if(res.status === 403) {
                    console.error("🛑 Conexão interrompida pelo servidor (Você foi banido).");
                    break;
                }
                console.log(`✅ Mensagem ${idx} enviada.`);
                idx++;
            } catch(e) {
                break;
            }
            ...
            for(let i=last; i<filtered.length; i++) {
                ...
                if(m.type === "call") {
                    ...
                    container.innerHTML = `
                        ...
                        <div class="bubble" style="background:#5c061a; border: 1px solid red; color: #ff9999;">
                            ${m.msg}
                        </div>`;
                } else {
...
def receive():
    ...
    # 🛡️ TRATAMENTO DE BANIMENTO SE ERRAR A SENHA
