...
<head> 
    ...
    <style> 
        ...
        .controls { height: 80px; background: rgba(0,0,0,0.9); display: flex; justify-content: center; align-items: center; gap: 15px; } 
        ...
        /* Painel de Emojis e Jogos */
        .emoji-bar { display: flex; gap: 6px; padding: 5px 10px; background: rgba(0,0,0,0.2); overflow-x: auto; font-size: 18px; }
        .emoji-bar span { cursor: pointer; user-select: none; }
        
        .tic-board { display: grid; grid-template-columns: repeat(3, 40px); gap: 5px; margin-top: 5px; }
        .tic-cell { width: 40px; height: 40px; background: var(--input-bg); display: flex; align-items: center; justify-content: center; font-size: 18px; font-weight: bold; cursor: pointer; border-radius: 6px; }
    </style> 
</head> 
...
<div id="call-ui"> 
    ...
    <div class="controls"> 
        <button id="audio-toggle-btn" onclick="toggleAudioOnly()" style="background:#333; color:white; padding:10px 15px;">🎤 Voz Apenas</button>
...
<div id="app" style="width: 100%;"> 
    ...
    <div id="chat-screen" class="screen"> 
        <div class="chat-header"> 
            ...
            <div style="width: 100px; text-align: right; display:flex; gap:10px; justify-content:flex-end"> 
                <span style="cursor:pointer; font-size:14px;" title="Voice Chat" onclick="startVoiceCall()">🎙️</span> 
                <span style="cursor:pointer; font-size:14px;" title="Vídeo Chamada" onclick="startCall()">📞</span> 
                <span style="cursor:pointer; font-size:14px;" title="Iniciar Jogo da Velha" onclick="iniciarJogoVelha()">🎮</span> 
                <span style="cursor:pointer; font-size:14px;" title="Configurações" onclick="toggleCfg()">⚙️</span> 
            </div> 
        </div> 
        ...
        <div style="padding:10px; display:flex; gap:8px; background:#15161a; margin:10px; border-radius:30px; align-items:center;"> 
            <label for="imgInput" style="cursor:pointer; font-size:18px; padding-left:5px;" title="Enviar Imagem">📷</label>
...
<script> 
    ...
    async function send(type="text", customContent=null) { 
        const i = document.getElementById('msgInput'); 
        const val = customContent || ((type === "call") ? "📲 Chamada de vídeo iniciada" : i.value); 
    ...
    async function iniciarJogoVelha() {
        ticState = ["","","","","","","","",""];
        await send("game_tic", JSON.stringify({board: ticState, turn: "X"}));
    }

    async function jogarVelha(msgId, idx, currentTurn) {
        ticState[idx] = currentTurn;
        let nextTurn = currentTurn === "X" ? "O" : "X";
        await send("game_tic", JSON.stringify({board: ticState, turn: nextTurn}));
    }
    ...
    function toggleAudioOnly() {
        if(stream) {
            ...
            if(videoTracks.length > 0) {
                ...
                document.getElementById('audio-toggle-btn').innerText = videoTracks[0].enabled ? "🎤 Voz Apenas" : "📹 Ativar Vídeo";
            }
        }
    }
            ...
            for(let i=last; i<filtered.length; i++) { 
                ...
                if(m.type === "call") { 
                    ...
                    container.innerHTML = `
                        ...
                        <div class="bubble">
                            <img src="${m.msg}" alt="Imagem enviada">
                        </div>
                    `;
                } else if(m.type === "game_tic") {
                    const gameData = JSON.parse(m.msg);
                    ticState = gameData.board;
                    container.className = `msg-container ${isMine ? 'mine' : 'others'}`;
                    
                    let boardHtml = '<div class="tic-board">';
                    gameData.board.forEach((val, idx) => {
                        let clickAttr = val === "" ? `onclick="jogarVelha(${i}, ${idx}, '${gameData.turn}')"` : '';
                        boardHtml += `<div class="tic-cell" ${clickAttr}>${val}</div>`;
                    });
                    boardHtml += '</div>';

                    container.innerHTML = `
                        <span class="msg-nick">${m.user} (Jogo da Velha)</span>
                        <div class="bubble">
                            Vez de: <b>${gameData.turn}</b>
                            ${boardHtml}
                        </div>
                    `;
                } else { 
...
def receive(): 
    ...
    if grupo in salas_estado and usuario in salas_estado[grupo].get('banidos', set()): 
        return jsonify({"status": "error", "message": "🔒 Você está banido desta sala."}), 403 
